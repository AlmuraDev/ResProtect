...
public class LightningListener implements Listener {
	...
	public void onLightningStrikeEvent(LightningStrikeEvent event) {
		...
		if (res != null) {
			if (!res.getPermissions().has("stormdamage", true)) {
				event.setCancelled(true);
			}
		} else if (!Residence.getWorldFlags().getPerms(event.getWorld().getName()).has("stormdamage", true)) {
			event.setCancelled(true);
		}
	}
}