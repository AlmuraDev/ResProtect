				...
				event.getPlayer()
						.sendMessage(
								ChatColor.GREEN
										+ "[ResProtect]"
										+ ChatColor.WHITE
										+ " You are not allowed to Chat in the current Residence location.");
				event.setCancelled(true);
			}
		} else if (!Residence.getWorldFlags()
				.getPerms(event.getPlayer().getWorld().getName())
				.has("chat", true)) {
			event.getPlayer().sendMessage(
					ChatColor.GREEN + "[ResProtect]" + ChatColor.WHITE
							+ " You are not allowed to Chat in current World.");
			event.setCancelled(true);
		}
	}
}